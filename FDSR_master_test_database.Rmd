...
### Assignment 1: probable questions

#### Which of the following will be will be mean of following numbers 26,91, 56,84,46,78, 67,87,72 and what will be the corresponding R code?
67.4, average(26,91, 56,84,46,78, 67,87,72)
64.7, mean(26,91,56,84,46,78, 67,87,72)
67.4, mean(nums) where nums is a numeric vector composed of 26,91, 56,84,46,78, 67,87,72
all of the above
a or c
b or c
none of the above
...
##### Explanation
```{r mm  }
nums <- c(26,91, 56,84,46,78, 67,87,72)
mean(nums)
```

#### Which of the following will be will be mean of following numbers 26,91, 56,84,46,78,NA,67,87,72 and what will be the corresponding R code?
67.4, average(26,91, 56,84,46,78,NA,67,87,72)
64.7, mean(26,91,56,84,46,78, NA, 67,87,72)
67.4, mean(nums,na.rm=t) where nums is a numeric vector composed of 26,91, 56,84,46,78,NA,67,87,72
all of the above
none of the above
...
##### Explanation
```{r mm  }
nums <- c(26,91, 56,84,46,78,NA, 67,87,72)
mean(nums,na.rm=T)
```

#### Now use the Life expectancy(LE) data and find the five point summary of life expectancy. How do you interpret the value of median? 
50% of the countries have life expectancy lower or equal to 73.2
50% of the countries have life expectancy higher or equal to 73.2
most  of the countries have life expectancy lower or equal to 73.2
both a and b
all of the above
none of the above
...
```
now let's find the five point summary

```{r fps  }
summary(life.exp.reg$le)
```
or we can also issue the following command

```{r med  }
median(life.exp.reg$le)
```


#### Now use the Life expectancy(LE) data and find the five point summary of life expectancy. How do you interpret the value of 1st quartile? 
75% of the countries have life expectancy higher than or equal to 64.6 years
25% of the countries have life expectancy less than or equal to 76.6 years
25% of the countries have life expectancy less than or equal to 64.6 years
both a and b
both a and c
none of the above
...
##### Explanation
We have already see in the above how to find five point summary and first quartile. An alternative way to find five point summary might be:
```{r quart  }
quantile(life.exp.reg$le,0.25)
```



#### Now use the Life expectancy(LE) data and find the five point summary of life expectancy. How do you interpret the value of third quartile?
25% of the countries have life expectancy less than or equal to 76.6 years
25% of the countries have life expectancy higher than or equal to 76.6 years
75% of the countries have life expectancy less than or equal to 76.6 years
both a and b
both b and c
none of the above
...
We have already see in the above how to find five point summary and first quartile. An alternative way to find five point summary might be:
```{r quart  }
quantile(life.exp.reg$le,0.75)
```

#### Suppose life.exp measures the life expectancy of all the countries in the world. The command boxplot(life.exp) will create a modified boxplot. True or False?
True
False
...
##### Explanation
when range is not specificed, then it uses default value of 1.5. It means that inter quartile range is multiplied by 1.5 to find the value of upper and lower whiskers. in the following i use the life expectancy data to show the difference between modified and regular boxplot.

```{r ss  }
par(mfrow=c(1,2))
bp.umod <- boxplot(life.exp.reg$le,range=0)
bp.mod <- boxplot(life.exp.reg$le)
bp.umod
bp.mod
```
here we do not see much of a difference
...
### TEST 16 (Week 7):(assignment 10,part 1) Probable questions
...
### TEST 17 (Week 7):(assignment 10,part 2) Probable questions
...
#### In theoretical world there is the true value of regression slope coefficient . In the data world, we have the corresponding value calculated from sample data. Which of the following is a valid question that can be raised about these two values?
...
####  If we deduct true population slope coefficient from sample slope coefficient and divide it by the standard error of sample slope coefficient, we get a 
...
#### Suppose we run a regression and then we plot the residuals against the explanatory variable. If we do not find any apparent relationship between residual and explanatory variable, then our linear regression may reflect the true relatioship between the variables. True or false?
True
False
Uncertain
...
#### If we find no apparent relationship between residuals and explanatory variable then it will imply
after taking out the regression component from the dependent variable, the rest of the effect is random in nature
after taking out the regression component from the dependent variable, the rest of the effect is not random in nature
both of the above
none of the above
...
#### If residual is systematically related to movements in explanatory variable then the current linear regression may not accurately describe the true nature of the relationship. True or False?
True
False
...
#### Consider the data in Data_LifeGDPhiv.txt  file. Draw a scatterplot between GDP per capita and Life expectancy of different countries and then fit a regression line through it. In such graph, which of the following will be a valid observation?
the  linear regression line  accurately describes relationship between life expectancy and GDP per capita
the  linear regression line  can not fully describes relationship between life expectancy and GDP per capita
the  linear regression line  can not at all describe relationship between life expectancy and GDP per capita
none of the above

##### Answer
the  linear regression line  can not fully descriobes relationship between life expectancy and GDP per capita
...
#### Now let's plot the residual of the above regression against GDP per capita and fit a regression line. Which of the following is a valid obersvation regarding this plot?
Poor countries, that is those countries with low GDP per capita are below the regression line
Rich countries, that is those countries with high GDP per capita are below the regression line
Middle income countries  are above the regression line
Residuals are not entirely random, there is some specific pattern present.
All of the above
None of the above
...
```{r resid  }
le.reg <- lm(le ~ gdppc, data=life.gdp.hiv)
res.x <- data.frame(gdppc=life.gdp.hiv$gdppc, resids=le.reg$residuals)
ggplot(data=res.x,aes(x=gdppc,resids)) +
      geom_point() + 
      geom_smooth(method="lm",se=FALSE) + 
	 xlim(0,50000) 
...
#### Consider the data in Data_LifeGDPhiv.txt  file. Run a regression between GDP per capita and Life expectancy of different countries(dependent variable). What is the value of R-square in this case?
0.403
0.304
0.430
none of the above
...
```{r sol  }
summary(le.reg)$r.squared
```

#### Consider the data in Data_LifeGDPhiv.txt  file. Non-linearity in the data when we plot life expectancy against GDP per capita can be mitigated by taking log of GDP per capita variable. True or False? 
True
False
...
#### Compare the standard deviation of life expectancy and GDP per capita variable. Which is one higher? 
GDPPC has higher standard deviation
Life expectancy has higher standard deviation
Not possible to determine standard deviation
None fo the above
...
```{r log  }
sd(life.gdp.hiv$le)
sd(life.gdp.hiv$gdppc)
```


#### How does standard deviation play a role in determining on which variable we should apply log transformation?
Usually log transformation is applied to  those variables which have high standard deviation
Usually log transformation is applied to those variables which have low standard deviation
Usually standard deviation does not play any role in determining log transformation
None of the above
...
#### Consider the data in Data_LifeGDPhiv.txt  file. If we now run a scatterplot of life expectancy against log of GDP per capita, what do you observe?
The relationship between these two variables has become more non-linear
The relationship between these two variables has become more linear
The relationship between these two variables has not been affected at all
None of the above
...
```{r sp  }
life.gdp.hiv$lgp <- log(life.gdp.hiv$gdppc)
ggplot(life.gdp.hiv, aes(x=lgp, y=le)) + 
		geom_point()
```
The scaterplot looks more linear now
...
#### Consider the data in Data_LifeGDPhiv.txt  file. Run a regression between log of GDP per capita and Life expectancy of different countries(dependent variable). What is the value of R-squared in this case?
0.403
0.657
0.430
none of the above
...
```{r sol  }
lle.reg <- lm(le ~ lgp, data=life.gdp.hiv)
summary(lle.reg)$r.squared
```
...
### TEST 18 (Week 8): (Inclass-quiz -5) Probable questions 
